...
function reverseString(str) {
  ...
  const reversed = array1.reverse().join('');
  return reversed;
}
...
function isPalindrome(str) {
  const array1 = str
    ...
    .split('');
  const reversed = array1.reverse().join('');
  if (reversed === str.toLowerCase().replace(/[^a-z]/g, '')) {
    return true;
  }
  return false;
}
...
function invertCase(str) {
  const chars = str
    .split('')
    .map(function invertCharCase(char) {
...
function getCardId(value) {
  ...
  const rankIndex =
    splitted.length === 3
      ? ranks.indexOf(splitted[0] + splitted[1])
      : ranks.indexOf(splitted[0]);
  const suitIndex =
    splitted.length === 3
      ? suits.indexOf(splitted[2])
      : suits.indexOf(splitted[1]);
